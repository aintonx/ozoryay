...
export const LAYOUT = {
  ...
  /** Прожектор — на гребне холма, в левой трети. */
  projectorX: 0.158,
 ...
 *
 * Крупные формы — гауссианы, поверх них мелкая неровность из наложенных
 * синусов. Гладкая дуга читается как геометрия, а не как земля: настоящий
 * горизонт всегда чуть рваный.
 */
export function groundYAt(x: number): number {
  ...
  // Три несоизмеримые частоты: узор не повторяется на глаз.
  const rough =
    Math.sin(x * 21.7 + 1.3) * 0.0016 +
    Math.sin(x * 47.3 + 4.1) * 0.0008 +
    Math.sin(x * 103.1 + 2.7) * 0.0004;

  return large + rough;
}

/**
 * Дальняя гряда — вторая линия холмов позади первой.
 *
 * Одна линия земли лишает кадр глубины: небо просто упирается в чёрное.
 * Вторая гряда, чуть выше и светлее, даёт воздух между планами.
 */
export function ridgeYAt(x: number): number {
  return (
    LAYOUT.groundY -
    0.052 -
    bump(x, 0.72, 0.34, 0.045) -
    bump(x, 0.3, 0.26, 0.022) +
    Math.sin(x * 15.3 + 0.7) * 0.0022
  );
}
