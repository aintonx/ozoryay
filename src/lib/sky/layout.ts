...
export const LAYOUT = {
  ...
  /**
   * Звезда «Созвездия Вечного Смеха» — маяк, к которому улетают с экрана
   * неба. Стоит на настоящем небе всегда, ещё до нажатия кнопки: полёт
   * должен нырять в то небо, что уже видно, а не подменять его на
   * неизвестное. В верхнем левом углу — подальше и от луны, и от вечной
   * звезды с прожектором в правой части, чтобы небо не собиралось в один угол.
   */
  journeyStar: { x: 0.24, y: 0.22 },
