...
export function drawComet(
  ...
  // Быстрое проявление, плавный уход в конце.
  const alpha = clamp01(p / 0.08) * (1 - clamp01((p - 0.78) / 0.22));
  ...
  const ahead = Math.min(1, head + 0.015);
  ...
  for (let i = 0; i < TAIL_STEPS; i++) {
    ...
    const dx = bx(Math.min(1, t + 0.01)) - bx(Math.max(0, t - 0.01));
    const dy = by(Math.min(1, t + 0.01)) - by(Math.max(0, t - 0.01));
