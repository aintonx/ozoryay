...
export function createSky(canvas: HTMLCanvasElement, initial: SkyOptions): SkyHandle {
  ...
  function frame(now: number) {
    ...
    // Звезда «Созвездия Вечного Смеха» — здесь же, рядом с обычными
    // звёздами-письмами, а не после общей заслонки (было ниже, вместе
    // с вечной звездой): она должна стоять на небе всегда, до всякого
    // нажатия кнопки, но это про то, что она не пропадает с экрана,
    // а не про то, что её не должно гасить письмо или прожектор. Раньше
    // оба этих смысла жили в одном месте кода и она заодно наследовала
    // и «не гаснет никогда» вечной звезды рядом — без всякой причины
    // для неё самой, только по соседству в коде. Здесь она рисуется
 ...
 * и заметнее: это маяк, до которого можно долететь, а не забытая лампа.
 * Стоит на экране всегда, до всякого нажатия кнопки, — полёт в
 ...
 *
 * В отличие от вечной звезды рядом, эта не исключение из правил
 * затухания: вызывается там же, где обычные звёзды-письма (см. `frame`
 * выше), и гаснет вместе с небом при открытом письме или работающем
 * прожекторе ровно так же, как они. Постоянно горящей она была
 * только по случайному соседству в коде с вечной звездой, а не по
 * смыслу — маяк, который не тускнеет, даже когда читаешь письмо
 * или смотришь фотографию, путал, а не помогал.
 */
