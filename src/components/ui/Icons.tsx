...
/**
 * Поцелуй: отпечаток губ. Единственная залитая иконка в наборе.
 *
 * Форма собрана по канону губного отпечатка: широкая верхняя губа с
 * выраженной ложбинкой и приподнятыми уголками, нижняя — полнее и короче,
 * между ними линия смыкания. Пропорция примерно два к одному: цельный
 * силуэт такой же высоты, как ширины, глаз читает как сердце.
 *
 * Та же форма летит кометой за горизонт — см. `drawKissMark` в sky/comet.
 */
export function IconKiss({ className, size }: IconProps) {
  return (
    <svg {...base(size)} className={className} stroke="none" fill="currentColor">
      {/* Верхняя губа: от уголка к всхолмию, ложбинка, второе всхолмие. */}
      <path d="M2.2 11.9C3.4 8.4 5.6 6.6 7.6 6.6C9.4 6.6 10.9 7.7 12 9.5C13.1 7.7 14.6 6.6 16.4 6.6C18.4 6.6 20.6 8.4 21.8 11.9C19.6 11.2 15.9 10.8 12 10.8C8.1 10.8 4.4 11.2 2.2 11.9Z" />
      {/* Нижняя: полнее, шире посередине, уголки подобраны кверху. */}
      <path d="M2.2 13.1C4.4 12.5 8.1 12.1 12 12.1C15.9 12.1 19.6 12.5 21.8 13.1C20.2 16.9 16.4 19.4 12 19.4C7.6 19.4 3.8 16.9 2.2 13.1Z" />
    </svg>
  );
}
