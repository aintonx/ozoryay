 ...
 *
 * Одно осознанное исключение из сетки 24×24 — `IconChevronUp` ниже: это
 * не квадратный символ, а плоская «сплющенная» галочка с фиксированным
 * соотношением сторон 4:1, своя история — см. комментарий прямо над ней.
 */
...
/**
 * Стрелка-подсказка о свайпе.
 *
 * Не квадратная, как остальные иконки этого файла, а плоская, «сплющенная»
 * галочка — та форма, которой iOS помечает Reachability (у Apple она
 * называется `chevron.compact.up`; та же идея под тем же именем есть
 * и в открытых наборах вроде Bootstrap Icons — `chevron-compact-up`).
 * Цифры ниже не придуманы на глаз: это прямой промер такой стрелки на
 * реальном скриншоте Reachability — у Apple она держит соотношение
 * сторон 4:1 и толщину штриха около 11% от ширины, что здесь и
 * воспроизведено (`viewBox` 44×11, штрих 4.8 из 44).
 *
 * Сам SF Symbol при этом не скопирован и не подключён — лицензия Apple
 * прямо запрещает переносить SF Symbols за пределы её платформ (см.
 * комментарий в начале файла); здесь самостоятельно построенная фигура
 * с той же геометрией, не файл из системного набора.
 *
 * `size` — ширина в пикселях; высота считается от неё через
 * зафиксированное соотношение 4:1 (`aspect-ratio`), а не приходит
 * отдельным параметром: квадратной сетки 24×24 у этой формы нет, потому
 * что сама фигура не квадратная.
 */
export function IconChevronUp({ className, size }: IconProps) {
  return (
    <svg
      ...
      style={{ width: size ?? "1em", aspectRatio: "44 / 11" }}
      className={className}
      aria-hidden="true"
    >
      <path d="M2.4 8.6L22 2.4L41.6 8.6" />
    </svg>
  );
}
