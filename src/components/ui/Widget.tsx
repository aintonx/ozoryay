...
export function WidgetButton({
  ...
  // строк.
  //
  // 14rem (была 13rem) — то же число, что у «Ты восхищаешь» и «Зоны»
  // (они зафиксированы в своём пределе и больше не растут, см. эти
  // виджеты); выросло вместе с ними на те же 1rem, синхронно, той же
  // правкой, что увеличила подписи внутри тайла ниже (`labelClasses`/
  // `hintClasses`) — крупнее текст просит больше воздуха вокруг себя,
  // и по той же причине у сетки на главном экране становится на эти
  // 1rem меньше свободного места снизу (см. `MAX_SCALE`/`fit()` в
  // `HomeScreen`: сетка не растёт сама выше своего натурального
  // размера, поэтому расти должны сами карточки). Раньше `tile` был на
  // 9.6rem — самой невысокой карточкой на всём экране, заметно ниже
  // остальных трёх. Теперь высота одинаковая у «Поцелуя», «Неба», «Ты
  // восхищаешь» и «Зоны» — четыре карточки на одной линии по высоте, и
  // только «Без тебя»/«Между нами» сверху остаются выше остальных как
  // более насыщенная, «геройская» пара — сознательная иерархия, а не
  // случайный разнобой. `justify-between` здесь же — иначе весь новый
  // рост просто осел бы пустым полем под подписью: значок держится
  // у верхнего края, подпись с текстом — у нижнего, а выросший запас
  // между ними становится гибким зазором, а не мёртвым полем.
  const layoutClasses =
    layout === "tile"
      ? "h-[14rem] flex-col items-start justify-between gap-[0.85rem] overflow-hidden text-left"
  ...
  // под это 14.5px/12px. `tile` (домашний экран) — крупнее (была
  // 17px/13.5px): жаловались именно на эти подписи, а не на `row`,
  // и здесь перенос строки не страшен — высота тайла зафиксирована
  // с запасом (см. `layoutClasses` выше), а не считается по содержимому.
  const labelClasses = layout === "tile" ? "text-[18px]" : "text-[14.5px]";
  const hintClasses = layout === "tile" ? "text-[14.5px]" : "text-[12px]";
