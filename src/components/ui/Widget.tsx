...
export function WidgetButton({
  ...
  // пара — сознательная иерархия, а не случайный разнобой.
  //
  // Эта высота НЕ трогается ради укрупнения шрифта ниже (`labelClasses`/
  // `hintClasses`), и это не случайно, а по опыту: `tile` зафиксирован
  // и обрезан (`overflow-hidden`) — что бы ни выросло внутри него, наружу,
  // в `natural`-высоту всей сетки на `HomeScreen`, это не просачивается.
  // Расти же сама высота — просачивается: подрос на `1rem`, а вместе с ней
  // подросла и «Зона» (`min-h-`, тоже без потолка) — и `natural` у сетки
  // перегнал `available` на экране автора задачи. `fit()` в ответ уменьшил
  // общий `scale`, тем же множителем ужав и только что увеличенный шрифт
  // обратно почти к прежнему размеру, а «Зону» — обрезал `overflow-hidden`
  // на `box`, потому что даже `MIN_SCALE` (тогда ещё существовавший пол —
  // с тех пор убран целиком, см. `HomeScreen`) не спас. Отсюда и «шрифт не
  // вырос», и «Зона заехала за чёрную полоску» разом, одной и той же
  // причиной. `justify-between` здесь же — иначе рост подписи ниже просто
  // осел бы пустым полем под ней: значок держится у верхнего края, подпись
  // с текстом — у нижнего, и между ними гибкий зазор, а не мёртвое поле.
  const layoutClasses =
    layout === "tile"
