...
export default function HomeScreen({
  ...
  const contentStyle = {
    ...
    // "center bottom", не "center center": `items-end` у `box` ниже кладёт
    // ЛЭЙАУТ-рамку `content` (его натуральный, ДО трансформации размер) по
    // нижнему краю — `transform` на расположение во флексе не влияет вообще,
    // только на то, как рамка потом рисуется на экране. Пока натуральная
    // высота хоть немного больше доступной, эта рамка нависает НАД `box`
    // сверху ровно на разницу — и `scale` от центра тогда делит компенсацию
    // пополам: верх съезжает вниз только на половину избытка, а низ —
    // настолько же вверх, открывая под собой зазор. Отсюда «Без тебя»/
    // «Между нами» заезжали под статус-бар, а снизу перед стрелкой всё
    // равно оставался зазор — то самое, ради чего стоит `items-end`, но им
    // же и перечёркнутое этой строкой. Точка отсчёта «снизу» устраняет это:
    // нижний край при сжатии остаётся на месте (там же, где его поставил
    // `items-end`), а весь излишек уходит только вверх, в запас над
    // «Без тебя»/«Между нами» — где он и должен быть.
    ...
    <div className="flex h-full w-full flex-col">
      ...
      >
        {/*
          ...
          перед стрелкой-подсказкой. `items-end` устраняет именно этот зазор
          снизу без единого лишнего пикселя `natural`-высоты: остаток уходит
          наверх, между верхним отступом безопасной зоны и «БЕЗ ТЕБЯ»/«МЕЖДУ
          НАМИ» — там он не виден как «недостроенный» интерфейс, а сама
          сетка, включая «Зону», всегда стоит вплотную к стрелке снизу.

          У этого решения была своя цена, и не сразу очевидная: `items-end`
          кладёт рамку `content` по её НАТУРАЛЬНОМУ (до `transform`) размеру —
          флекс-раскладку сам `transform: scale()` не видит и не трогает,
          он только перерисовывает уже готовую рамку поверх. Пока натуральная
          высота хоть немного больше доступной, эта рамка нависала над `box`
          сверху — а `transformOrigin` у `content` стоял `"center center"`,
          то есть сжатие шло от середины и делило нависшее пополам: половина
          уходила вниз, в границы `box`, другая половина оставалась торчать
          сверху — и `overflow-hidden` эту верхнюю половину обрезал, ровно
          на «Без тебя»/«Между нами». А снизу освобождалось место на ту же
          величину — тот самый зазор перед стрелкой, который `items-end`
          должен был устранить, а не переоткрыть с другой стороны. Правка —
          `transformOrigin: "center bottom"` у `content` (см. ниже): нижний
          край сжимается на месте, весь излишек уходит только вверх, туда же,
          куда его и укладывает `items-end`. Обе половины одной и той же
          идеи должны стоять вместе — сепаратно друг без друга работают
          лишь наполовину.
